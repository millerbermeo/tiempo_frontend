...
export const RegistrarCompeticion = ({ fetchData }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await axiosClient.post('competicion/', formData).then((response) => {
                setIsSuccess(true);
                fetchData();
                onOpenChange(false);
                setFormData({
                ...
                setMessage('Competición Registrada Con Éxito');
            });
        } catch (error) {
            console.error('Error al enviar los datos:', error);
            setIsSuccess(false);
            setMessage('Competición No Registrada');
        }
    };

    return (
        <div className="flex flex-col gap-2">
            <Button color="primary" endContent={<PlusIcon />} onPress={onOpen}>Registrar Competición</Button>
