...
export const TableCategorias = () => {
  ...
  const convertToCSV = (data) => {
    const headers = ["ID", "NOMBRE", "COMPETENCIA", "FECHA CREACIÓN", "FECHA MODIFICACIÓN"];
    const rows = data.map(item =>
      [item.id, item.nombre, item.fk_competencia.nombre, item.date_created, item.date_modified].join(',')
    );
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-center my-5">
        <span className="text-default-400 text-small">Total {data.length} categorías</span>
        ...
        <RegistrarCategoria fetchData={fetchData}/>
      </div>
        ...
        <TableBody>
          {paginatedData.map(item => (
            <TableRow key={item.id}>
              ...
              <TableCell>{item.fk_competencia.nombre}</TableCell>
              <TableCell>{item.date_created}</TableCell>
              <TableCell>{item.date_modified}</TableCell>
              <TableCell>
                <ActualizarCategoria fetchData={fetchData} categoria={item}/>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
